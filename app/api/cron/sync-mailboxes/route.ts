...
export async function GET(req: Request) {
  ...
  if (!env.MAILBOX_SECRET_KEY) return Response.json({ skipped: "mailbox-secret-key-missing" });

  const runId = await getBackgroundTaskService().dispatchTracked("sync-mailboxes", {});
