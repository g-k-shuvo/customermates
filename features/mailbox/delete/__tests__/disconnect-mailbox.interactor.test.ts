...
import { DisconnectMailboxInteractor } from "../disconnect-mailbox.interactor";
...
function interactorFor(existing: unknown) {
  const findConnectedMailbox = vi.fn().mockResolvedValue(existing);
  const deleteConnectedMailbox = vi.fn().mockResolvedValue(undefined);

  return {
    interactor: new DisconnectMailboxInteractor({ findConnectedMailbox, deleteConnectedMailbox } as never),
    findConnectedMailbox,
    deleteConnectedMailbox,
  };
}

describe("DisconnectMailboxInteractor", () => {
  it("removes the connected account and answers with its id", async () => {
    const { interactor, deleteConnectedMailbox } = interactorFor({ connectedAccountId: ACCOUNT_ID });

    const result = await interactor.invoke({ connectedAccountId: ACCOUNT_ID });

    expect(deleteConnectedMailbox).toHaveBeenCalledWith(ACCOUNT_ID);
    expect(result.ok).toBe(true);
    expect(result.ok && result.data).toBe(ACCOUNT_ID);
  });

  it("reports an unknown mailbox as not found and deletes nothing", async () => {
    const { interactor, deleteConnectedMailbox } = interactorFor(null);

    const result = await interactor.invoke({ connectedAccountId: ACCOUNT_ID });

    expect(deleteConnectedMailbox).not.toHaveBeenCalled();
  ...
  it("rejects an id that is not a uuid before touching the repository", async () => {
    const { interactor, findConnectedMailbox } = interactorFor({ connectedAccountId: ACCOUNT_ID });

    const result = await interactor.invoke({ connectedAccountId: "not-a-uuid" });

    expect(findConnectedMailbox).not.toHaveBeenCalled();
    expect(result.ok).toBe(false);
  });
});
